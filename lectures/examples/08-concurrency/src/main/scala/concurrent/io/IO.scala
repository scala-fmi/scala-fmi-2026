...
trait IO[+A]:
  ...
  infix def zip[B](that: IO[B]): IO[(A, B)] = Zip(this, that)
...
object IO:
  ...
  def fromFuture[A](fa: => scala.concurrent.Future[A]): IO[A] = usingCallback[A] { (ec, callback) =>
    fa.onComplete(callback)(using ec)
  }
  ...
  def unsafeRunAsync[T](io: IO[T])(onComplete: Callback[T])(ec: ExecutionContext): Unit =
    ...
    io match
      ...
      case zipIo @ Zip(ioA, ioB) =>
        ...
        def completeA(aResult: Try[zipIo.AType]): Unit =
...
//case class BindTo[A](ioA: IO[A], ec: ExecutionContext) extends IO[A]
//
//type Callback[-A] = Try[A] => Unit
//
//object IO:
