...
object LibraryApi:
  private def retrieve(path: String) =
    HttpClient.getCatsIO(s"http://localhost:8080/$path").flatMap { response =>
      ...
      else if response.getStatusCode >= 400 && response.getStatusCode < 500 then
        IO.raiseError(BadResponse(response.getStatusCode, response.getResponseBody))
      else IO.raiseError(new RuntimeException("Something happened..."))
    }
  ...
  def retrieveBook(bookId: String): IO[Book] =
    for
      (name, genre, authorsId) <- (
        retrieve(s"books/$bookId/name"),
        retrieve(s"books/$bookId/genre"),
        retrieve(s"books/$bookId/authors")
      ).parTupled
      authors <- asList(authorsId).parTraverse(retrieveAuthor)
...
object LibraryClient extends IOApp.Simple:
  ...
  def retrieveAndDisplayBook(bookId: String): IO[Unit] =
    (for
      book <- LibraryApi.retrieveBook(bookId)
      _ <- displayBook(book)
    yield ()).recoverWith:
      case BadResponse(404, _) =>
        IO.println(s"Book $bookId not found, please select another book")
    // TODO: what if the book is not found?
//      .recoverWith:
