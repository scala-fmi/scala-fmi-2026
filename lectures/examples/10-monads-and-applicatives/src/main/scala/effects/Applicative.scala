...
trait Applicative[F[_]] extends Functor[F]:
  ...
  // Derived operations:
  extension [A](fa: F[A]) def map[B](f: A => B): F[B] = map2(fa, unit(()))((a, _) => f(a))
  ...
  extension [A, B](tuple: (F[A], F[B]))
    def zipN: F[(A, B)] =
      val (fa, fb) = tuple
      map2(fa, fb)((_, _))

    def mapN[R](f: (A, B) => R): F[R] = tuple.zipN.map(f.tupled)

  extension [A, B, C](tuple: (F[A], F[B], F[C]))
    def zipN: F[(A, B, C)] =
      val (fa, fb, fc) = tuple
      map3(fa, fb, fc)((_, _, _))

    def mapN[R](f: (A, B, C) => R): F[R] = tuple.zipN.map(f.tupled)

  extension [A, B, C, D](tuple: (F[A], F[B], F[C], F[D]))
    def zipN: F[(A, B, C, D)] =
      val (fa, fb, fc, fd) = tuple
      map4(fa, fb, fc, fd)((_, _, _, _))

    def mapN[R](f: (A, B, C, D) => R): F[R] = tuple.zipN.map(f.tupled)
