...
trait Monad[F[_]] extends Applicative[F]:
  ...
  extension [A](fa: F[A]) override def map[B](f: A => B): F[B] = fa.flatMap(a => unit(f(a)))
...
object Monad:
  ...
  given Monad[Try]:
    ...
    extension [A](m: Try[A]) def flatMap[B](f: A => Try[B]): Try[B] = m flatMap f

  given Monad[List]:
    def unit[A](a: A): List[A] = List(a)

    extension [A](m: List[A]) def flatMap[B](f: A => List[B]): List[B] = m flatMap f

  // givens can accept context parameters
  given (ec: ExecutionContext) => Monad[Future]:
    ...
    extension [A](m: Future[A]) def flatMap[B](f: A => Future[B]): Future[B] = m flatMap f

  given [E] => Monad[[A] =>> Either[E, A]]:
    def unit[A](a: A): Either[E, A] = Right(a)

    extension [A](fa: Either[E, A]) def flatMap[B](f: A => Either[E, B]): Either[E, B] = fa.flatMap(f)
