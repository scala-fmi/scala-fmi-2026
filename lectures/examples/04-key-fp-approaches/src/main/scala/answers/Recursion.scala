...
object MoreListFunctions:
  ...
  def take[A](la: List[A], n: Int): List[A] =
    @tailrec
    def take(rest: List[A], k: Int, acc: List[A] = Nil): List[A] =
      if k == 0 then reverse(acc)
      else take(rest.tail, k - 1, rest.head :: acc)
  ...
  def concat(l1: List[Int], l2: List[Int]): List[Int] =
    @tailrec
    def concat(rest: List[Int], acc: List[Int]): List[Int] =
      if rest.isEmpty then acc
      else concat(rest.tail, rest.head :: acc)

    concat(reverse(l1), l2)
