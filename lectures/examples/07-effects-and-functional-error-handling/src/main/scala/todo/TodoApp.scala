...
object TodoApp:
  ...
  def completeTodo(todoList: TodoList): IO[Option[TodoListAction]] = for
    ...
    maybeAction <-
      ...
      else IO.println(s"Non-existent todo $id") >> IO.of(None)
  yield maybeAction
  ...
  def todoListApp(todoList: TodoList): IO[Unit] = for
    ...
    updatedTodoList = maybeAction.map(applyTodoListAction(todoList)).getOrElse(todoList)
