...
class LibraryController(library: Library):
  ...
  val retrieveAuthors = LibraryEndpoints.retrieveAuthorsEndpoint.serverLogicSuccess: _ =>
    library.allAuthors

  val retrieveAuthor = LibraryEndpoints.retrieveAuthorEndpoint.serverLogic: authorId =>
    library.findAuthor(authorId).map(_.toRight(s"Author $authorId not found)"))

  val endpoints = List(retrieveBook, retrieveBooks, retrieveAuthors, retrieveAuthor)
