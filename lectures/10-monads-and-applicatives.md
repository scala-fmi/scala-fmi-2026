...
* Частичност/частични функции – `Option[A]`
* Изключения/грешки – `Try[A]`/`Either[E, A]`
* Недетерминизъм/множественост – `List[A]`
* Логване/мониторинг – `Writer[A]`
* Изменимо състояние – `State[S, A]`
* Вход/изход – `IO[A]`
* Асинхронност и конкурентност – `IO[A]`, `Future[A]`
* Ресурси – `Resource[E, A]`
...
Това също може да се генерализира като използваме `MonadError[F[_], E]`. Ще видим по-късно дефиницията в Cats
...
Алтернативни базови операции са:
...
* В дефиницията на `sequence` & `traverse` виждаме конкретен тип (`List`), който може да бъде генерализиран
