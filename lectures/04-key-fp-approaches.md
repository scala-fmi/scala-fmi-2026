...
* всъщност се дефинира обект от тип `Function2` с метод `apply`
  ...
  ```
* [`Function2`](https://www.scala-lang.org/api/current/scala/Function2.html) e нормален `trait` - репрезентира функции на два аргумента
...
// Не важи за функции без параметри
def sayHi = println("Hi")

val hiFunc = sayHi // директно извиква sayHi и връща ()
...
# Когато работим с изменими структури
...
# Неизменими структури

```scala
class Santase(val deck: List[Card], ... /* other state */):
  // game logic here
```

::: { .fragment }

Третираме `deck` просто като стойност
...
* Популяризирани чрез Clojure и Scala
  ...
  - Преди (20-ти век): значително по-скъпи памет и storage спрямо наличната изчислителна мощ
...
* Append-only logs
  - Write-ahead logging и Log-structured merge-tree са широко използвани в базите от данни (SQL и NoSQL)
...
* Copy-on-write и snapshot- в съвременните файлови системи (Btrfs, ZFS, APFS)

:::

# Други приложения – Git

[![](images/04-key-fp-approaches/git-xkcd.png){ height=480 }](https://xkcd.com/1597/)
...
![](images/04-key-fp-approaches/git-objects-0.png){ height=320 }

# Други приложения – Git

![](images/04-key-fp-approaches/git-objects-1.png){ height=540 }

# Други приложения – Git

![](images/04-key-fp-approaches/git-objects-2.png){ height=540 }
...
(1, "Two", 3.0).toList
// val res: List[Int | (String | Double)] = List(1, Two, 3.0)
```
